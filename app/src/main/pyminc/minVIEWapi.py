...
class MinControlAPI(object):
    def __init__(self):
        ...
        #self.helper.connect()
        #self.minIONdict=minIONdict
        #self.minIONdict_test=minIONdict_test
        #self.minIONclassdict=minIONclassdict
        self.counter=0
        #print "Setting up to process Shizzle"
        thread = threading.Thread(target=self.process_shizzle, args=())
        thread.daemon = True                            # Daemonize thread
        thread.start()

    def process_shizzle(self):
        while True:
            #print "processing shizzle"
            ...
            #                                        check_email_twitter()
                    ...
                    if minIONdict[minION]["state"]=="active": #we have a sequencing minION - what is it doing?
                        ...
                        try:
                            ...
                            if "connected" not in minIONclassdict[minION]:
                                try:
                                    minIONclassdict[minION]["class"].connect()
                                #    print "getting logged on 1"
                                    minIONclassdict[minION]["class2"].connect()
                                #    print "getting logged on 2"
                                except Exception, err:
                                    print "Connection failed 883", err
                                try:
                                #    print "GETTING THE GOOD STUFF"
                                    minIONclassdict[minION]["class"].init_minion(minION)
                                #    print "init 1"
                                    minIONclassdict[minION]["class2"].init_minion(minION)
                                #    print "init 2"
                                    results = execute_command_as_string(commands('get_analysis_configuration'), ipadd,int(minIONdict[minION]["port"]))
                                #    print "got results"
                                #    print results["result"]["channel_states"]
                                    if "channelstuff" not in minIONdict[minION]:
                                        minIONdict[minION]["channelstuff"]=dict()
                                    #print minIONdict[minION]["channelstuff"]
                                    #print "results", results
                                    minIONdict[minION]["channelstuff"]=results["result"]["channel_states"]
                                    #print "is this what failes?"
                                except Exception, err:
                                    print "Connection failed 895", err
                                minIONclassdict[minION]["connected"]="True"
                                #print "line 1025"
                            ...
                            minIONdict[minION]["scripts"]=get_run_scripts(minIONdict[minION]["port"])
                            for query in ('status','dataset','biasvoltageget','bias_voltage_gain','machine_id','machine_name','sample_id','user_error','sequenced_res','yield_res','current_script','disk_space','flow_cell_id'):#,'getstaticdata','get_analysis_configuration'):
                                results = execute_command_as_string(commands(query), ipadd, int(minIONdict[minION]["port"]))
                                livedata[query]=results
                                try:
                                    if query == "current_script":
                                        #print minION,"current_script"
                                        #print results
                                        #print self.minIONclassdict[minION]["class"]
                                        #print runname
                ...
                #                        check_email_twitter()
                                    ...
                                    print "Pain failed 924", err

                                results2 = execute_command_as_string(commands('get_analysis_configuration'), ipadd, int(minIONdict[minION]["port"]))
                                #print results2
                ...
                if "yield_res" in minIONdict[minion]["livedata"]:
                    if "result" in minIONdict[minion]["livedata"]["yield_res"]:
                        ...
                        try:
                            ...
                            heatsinktemp = 0
                        #print "Heatsinktemp",heatsinktemp
